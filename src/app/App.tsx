...
import { HarveyBallInput } from './components/harvey-ball-input';
import { WireframePrototype } from './components/wireframe-prototype';
import { PhaseProgress } from './components/phase-progress';
import { Participant } from './components/participant-list';
import { useVotePolling } from './hooks/useVotePolling';

const BASE_PARTICIPANTS: Participant[] = [
  { id: 'p1', name: 'Anna S.', initials: 'AS', vote: 4 },
  { id: 'p2', name: 'Ben K.', initials: 'BK', vote: 4 },
  { id: 'p3', name: 'Chiara L.', initials: 'CL', vote: 3 },
  { id: 'p4', name: 'David M.', initials: 'DM', vote: 3 },
  { id: 'p5', name: 'Elena V.', initials: 'EV', vote: null },
  { id: 'p6', name: 'Fatima R.', initials: 'FR', vote: null },
  { id: 'p7', name: 'Greg T.', initials: 'GT', vote: null },
];
...
export default function App() {
  ...
  const handleVotesUpdate = useCallback((incoming: Record<string, number>) => {
    setConnected(true);
    setApiVotes(incoming);
  }, []);

  useVotePolling(handleVotesUpdate);

  const participants: Participant[] = BASE_PARTICIPANTS.map(p => ({
    ...p,
    vote: apiVotes[p.id] ?? p.vote,
  }));

  const votedCount = participants.filter(p => p.vote !== null).length;
  const myVote = apiVotes['p5'] ?? null;

  const voteTotals = [1, 2, 3, 4].map(v =>
    participants.filter(p => p.vote === v).length
  );
  const totalVotes = Math.max(1, votedCount);
  const distribution = voteTotals.map((count, index) => ({
    vote: index + 1,
      ...
      <div className="grid grid-cols-[220px_580px_1fr] gap-6 flex-1 min-h-0">
        ...
        <div className="space-y-4 overflow-hidden flex flex-col">
          ...
          <div className="border-2 p-4 bg-white space-y-4 flex-1" style={{ borderColor: '#2E5284' }}>
            ...
            <VotingChart distribution={distribution} />
          ...
          <div className="flex gap-4 flex-shrink-0">
            <HarveyBallInput value={myVote} connected={connected} />
            ...
            <div className="border-2 p-3 bg-white space-y-2 flex-1 flex flex-col justify-between" style={{ borderColor: '#2E5284' }}>
              ...
              <div className="font-['Space_Mono'] font-bold text-7xl text-center">{votedCount}/{participants.length}</div>
